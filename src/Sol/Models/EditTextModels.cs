...
public sealed record CalculationResult(
    IReadOnlyList<string> LineOutputs,
    double Sum,
    double Average,
    int ErrorCount,
    int EvaluatedCount);

public sealed class EditTextOpenEventArgs : EventArgs
{
    public string? InitialText { get; }
    public EditTextTableDocument? InitialTable { get; }

    public EditTextOpenEventArgs(string? initialText = null, EditTextTableDocument? initialTable = null)
    {
...
public sealed class EditTextTableDocument
{
    ...
    public string SerializeToMarkdown()
    {
        ...
        for (int r = 0; r < RowCount; r++)
        {
            ...
            for (int c = 0; c < ColumnCount; c++)
            {
                string val = c < Rows[r].Count ? (Rows[r][c] ?? string.Empty).Replace("|", "\\|") : string.Empty;
