...
public class EditTextTableDocumentTests
{
    ...
    [Fact]
    public void SerializeToMarkdown_FormatsCleanMarkdownTable()
    {
        EditTextTableDocument document = EditTextTableDocument.CreateFromText("Product\tPrice\r\nWidget\t$10");
        string markdown = document.SerializeToMarkdown();

        Assert.Contains("| Column A | Column B |", markdown);
        Assert.Contains("| --- | --- |", markdown);
        Assert.Contains("| Product | Price |", markdown);
        Assert.Contains("| Widget | $10 |", markdown);
    }
}
